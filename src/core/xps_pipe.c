...
xps_pipe_t *xps_pipe_create(xps_core_t *core, size_t buff_thresh,
	...
	xps_pipe_t *pipe = (xps_pipe_t *)malloc(sizeof(xps_pipe_t));
...
void xps_pipe_destroy(xps_pipe_t *pipe) {
	...
	for (int i = 0; i < pipes->length; i++) {
		xps_pipe_t *cur = (xps_pipe_t *)pipes->data[i];
		if (cur == pipe) {
			pipes->data[i] = NULL;
...
/*
 * source / sink attach / detach functions
 */
...
xps_pipe_source_t *xps_pipe_source_create(void *ptr, xps_handler_t handler_cb,
	...
	xps_pipe_source_t *source =
		(xps_pipe_source_t *)malloc(sizeof(xps_pipe_source_t));
	if (source == NULL) {
		logger(LOG_ERROR, "xps_pipe_source_create()",
			   "malloc() failed for 'source'");
		return NULL;
	}
	...
	logger(LOG_DEBUG, "xps_pipe_source_create()", "created pipe_source");
...
xps_pipe_sink_t *xps_pipe_sink_create(void *ptr, xps_handler_t handler_cb,
	...
	xps_pipe_sink_t *sink = (xps_pipe_sink_t *)malloc(sizeof(xps_pipe_sink_t));
	if (sink == NULL) {
		logger(LOG_ERROR, "xps_pipe_source_create()",
			   "malloc() failed for 'source'");
		return NULL;
